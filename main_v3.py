...
class SpineHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if os.path.isfile(requested_path):
            try:
                # Đọc file ở chế độ BINARY để truyền tải dữ liệu
            ...
            for file in files:
                ...
                if file.endswith(".json"):
                    ...
                    if atlas_file:
                        ...
                        # Đọc trực tiếp trường version trong JSON
                        try:
                            with open(full_file_path, 'r', encoding='utf-8') as f:
                                data = json.load(f)
                                if 'skeleton' in data and 'spine' in data['skeleton']:
                                    spine_version = data['skeleton']['spine']
                        except Exception as e:
...
if __name__ == '__main__':
    ...
    html_path = get_resource_path("index_v3.html")
